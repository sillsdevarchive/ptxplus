...
class UpdateProjectSettings (object) :
	...
	def main (self) :
		...
		try :
			curVer = settings['General']['systemVersion']
		...
		if settings['System']['systemVersion'] != curVer :
			...
			if os.path.isfile(tempMasterConfFile) != True :
				...
				masterSettings['General']['systemVersion'] = settings['System']['systemVersion']

				# Now we will write out the results to our new master copy
				masterSettings.write()

				tools.userMessage("Info: Version numbers did not match, project was updated from: " + curVer + " to: " + masterSettings['General']['systemVersion'])
				return True
		else :
