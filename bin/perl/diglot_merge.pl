...
##
#		if ($isheading{$code}) { #Section header
#			$heading[$side]=$cl;
#			my ($thiscode,$ok);
#			$ok=1;
#			logit($side,"Swallowing section heading");
#			while ($#{$data[$side]}>=0 and $ok==1) {
#				if((${$data[$side]}[0])=~/^\\(\S+)/) {
#					$thiscode=$1;
#					if (!$active{$thiscode} or $splitcode{$thiscode}) {
#						$heading[$side].=shift @{$data[$side]};
#					} else {
#						$ok=0;
#					}
#				} else {
...
#		}
#		if ($active{$code}) {
#			logit($side,"CHECKING:",$code,$mode,$side,$oldpos);
#			if ($cleversections and $position[$side] !~ /$sectioncheck/o and $oldpos =~ /$sectioncheck/o) {
#				if($inrange[$side]) {
#					if ($position[$otherside] gt $position[$side]) {
#						logit($side,"Causing clever sectioning",$oldpos,$sectioncheck);
#						if ($code=~/^[cv]$/) {
#							$hold[$side].=$cl;
#						} else {
#							$chunk[$side].=$cl;
...
#						next;
#					#} else {
#						#logit($side,"Clever sectioning: Requesting break on other side at $oldpos");
#						#$breakafter{$oldpos}=1;
#						#if ($code eq 'p') {
#							#$chunk[$side].=$cl ;
#						#} else {
...
#					}
#				}
#				#$position[$otherside]=$position[$side];
#			}
#			if (defined($splitcode{$code}) and $mode=~/[lcr]/) {
#				if($inrange[$side]) {
#					if ($isheading{$code}) { #Section header
#						my ($thiscode,$ok);
#						if ($heading[$otherside]) {
#							logit($side,"Outputing existing chunks and both section headings");
#							do_output($position[$side],\@chunk);
#							do_output($position[$side],\@heading);
#							next;
#						} else {
#							logit($side,"Checking other side",${$data[$otherside]}[0]);
#							$ok=1;
#							if (${$data[$otherside]}[0]=~/^\\(\S+)/  or ${$data[$otherside]}[1]=~/^\\(\S+)/) {
#								$thiscode=$1;
#								if ($active{$thiscode} and !$splitcode{$thiscode}) {
#									$ok=0;
#								}
#							}
#							if ($ok) {
#								logit($side,"Switching sides to see if there's a mirror heading");
#								$otherside=$side;
#								$side++;
#								$side%=2;
#								next;
#							} else {
#								if ($position[$otherside] ge $position[$side]) {
#								  logit($side,"Outputing existing chunks and this side's headings");
#								  do_output($position[$side],\@chunk);
#								  do_output($position[$side],\@heading);
#								  next;
#								}
#								logit($side,"regurgitating heading");
#								$cl=$heading[$side];
#								$heading[$side]='';
#							}
#						}
#					} elsif (($side==0 and $mode eq 'l') or ($side==1 and $mode eq 'r')) {
#						if ($inrange[$side]) {
#							my $ok=1;
#							foreach my $search (@required) {
#								if ($position[$side] !~ /$search/) {
#									$ok=0;
#									logit($side,$search,"not found in",$position[$side]);
#								}
#							}
#							if ($ok) {
#								if ($position[$otherside] gt $position[$side]) {
...
#									}
#									#$chunk[$otherside].=$cl;
#									logit($side,"Forcing paragraph");
#									do_output($position[$side],\@chunk);
#									next;
#								} else {
#									logit($side,"Paragraph: Requesting break on other side at $oldpos");
#									$breakafter{$oldpos}=1;
#									if ($code eq 'p') {
#										$chunk[$side].=$cl ;
#									} else {
...
#				}
#				if ($breakafter{$oldpos}) {
#					logit($side,"Granting requested break");
#					do_output($position[$side],\@chunk);
#					$hold[$side].=$cl;
#					$otherside=$side;
#					$side++;
...
#				}
#				# Standard case...
#				if ($oldpos ne $position[$side] and  $position[0] eq $position[1]) {
#					my $ok=1;
#					foreach my $search (@required) {
#						if ($position[0] !~ /$search/) {
#							$ok=0;
#							logit($side,$search,"not found in",$position[0]);
#						}
#					}
#					if ($ok) {
#						logit($side,'Outputting text');
#						if ($code eq 'p') {
#							if ($inrange[$side]) {
#								$chunk[$side].=$cl ;
#								do_output($position[$side],\@chunk);
...
#			}
#		}
#		if ($position[$side] ge $position[$otherside]) {
#			#if ($code =~ /^[cv]$/ and $#{$data[$otherside]}>0)  # Put verse marker back before switching sides
#			if ($code ne "p" and $#{$data[$otherside]}>0) { # Put verse marker back before switching sides
#				if ($inrange[$side]) {
#
#					logit($side,"holding",$cl);
#					$hold[$side].=$cl;
#				}
#			} else {
#				$chunk[$side].=$cl if($inrange[$side]);
#			}
#			$cl="";
#			}
#		}
...
#	if ($inrange[$side]) {
#		if ($hold[$side]) {
#			logit($side,"using held value");
#		}
#		$chunk[$side].=$hold[$side].$cl ;
#		$hold[$side]="";
#		$cl="";
#	} else {
#		if ($hold[$side]) {
#			$hold[$side]="";
#			logit($side,"junking held value");
#		}
#	}
#}
#
